...
const ActionCreators = {
  selectItem : (id) => ({type: Actions.SELECT_ITEM, id})
}
...
class ZeldaItemSelector extends React.PureComponent {
  constructor(props) {
    ...
    const selectedItemId = Selectors.getSelectedItemId(storeState)
    this.state = {
  ...
  render () {
    ...
    const items = Selectors.getItems(store.getState())
    const selectedItem = selectedItemId ? find(items, {id: selectedItemId}) : null
    return <div>
      <h1>Zelda items selector 2</h1>
