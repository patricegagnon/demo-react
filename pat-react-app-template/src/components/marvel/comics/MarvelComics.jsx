...
class MarvelComics extends React.Component {
  ...
  fetchPageData(offset, limit) {
    ...
    this.marvelService.getComics(offset, limit).then((response) => {
      this.props.actions.setComics( response.results, response.offset, response.limit, response.total)
    }).catch(error => {
  ...
  renderComicTile(comic) {
    return <div key={`comic${comic.id}`} className="card m-2" style={{width: '300px'}}>
      ...
      <div className="card-body">
        <h5 className="card-title">{this.props.showDetailLink ?
          <Link to={`/comics/${comic.id}`}>{comic.title}</Link> : comic.title}</h5>
      </div>
    </div>
  }
